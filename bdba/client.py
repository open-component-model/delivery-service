...
def dash_to_snake_case_keys(d: dict[str, object]) -> dict:
    '''
    Highly opinionated function to parse the BDBA analysis result so that it can be processed by the
    our BDBA model classes. In that, it converts dash-cased keys into snake_case (as expected by our
    model classes).
    '''
    result = {}
    ...
    for key, value in d.items():
        if isinstance(value, dict):
            value = dash_to_snake_case_keys(value)
        elif isinstance(value, list):
            value = [
                dash_to_snake_case_keys(v) if isinstance (v, dict) else v
                for v in value
            ]
...
class BDBAApi:
    ...
    def upload(self,
        ...
        return dacite.from_dict(
            ...
            data=dash_to_snake_case_keys(result),
