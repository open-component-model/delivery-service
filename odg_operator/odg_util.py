...
def patch_jsonpath_into_dict(
    ...
    This function takes a JSONPath-like string (using dot notation, with support for quoted keys containing dots),
    a value to insert, and an optional input dictionary. It traverses or creates the nested dictionary structure
    according to the path, and sets the value at the specified location.
    Args:
        jsonpath (str): The JSONPath-like string specifying the nested keys, e.g., 'foo.bar."baz.qux"'.
    ...
    Example:
        >>> patch_jsonpath_into_dict('foo.bar."baz.qux"', 42)
        {'foo': {'bar': {'baz.qux': 42}}}
    '''
...
def template_and_resolve_jsonpath(
    ...
    Args:
        ...
        `substituion_context`: dictionary for placeholder substitution.
    ...
    for entry in value if isinstance(value, list) else [value]:
        templated = string.Template(entry).substitute(substituion_context)
