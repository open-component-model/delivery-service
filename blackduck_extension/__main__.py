...
def scan(
    ...
    **kwargs,
    ...
    component_artifact_metadata = bdba_utils.util.component_artifact_metadata(resource_node)
    target_product_id = bdba_utils.util._matching_analysis_result_id(
        component_artifact_metadata, known_scan_results
    )
    ...
    if not target_product_id:
        ...
        # discard first item - does not contain product_id
        _ = next(scan_result, None)

        target_product_id = next(scan_result, None).data.product_id

        print(target_product_id)

    bdio: bdba.model.BDIO = bdba_client.bdio_export(product_id=target_product_id)

    project = blackduck_extension.util.find_project_by_name(
    ...
    blackduck_client.create_project(project_name=bdio.name)
