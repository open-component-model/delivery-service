...
class SearchCriterion(enum.StrEnum):
    OCM = 'ocm'
    ARTEFACT_METADATA = 'artefact-metadata'
    FULLTEXT = 'fulltext'


class MatchingMode(enum.StrEnum):
    INCLUDE = 'include'
    EXCLUDE = 'exclude'


class SearchQueryOperator(enum.StrEnum):
    EQ = 'eq'
    IN = 'in'
    RANGE = 'range'
    CMP = 'cmp'
...
class Criterion:
    ...
    @staticmethod
    def _parse_mode(entry: dict) -> MatchingMode:
        mode_raw = entry.get('mode')
        if mode_raw is None or mode_raw == '':
            return MatchingMode.INCLUDE
        try:
            return MatchingMode(str(mode_raw).lower())
        except Exception:
...
class OcmCriterion(Criterion):
    ...
    type: SearchCriterion = dataclasses.field(default=SearchCriterion.OCM, init=False)
    ...
    def from_dict(cls, entry: dict) -> typing.Self:
        ...
        if not value:
            raise ValueError('ocm entry missing value')

        if not isinstance(recursive, bool):
            raise ValueError('ocm recursive must be a boolean')
...
class ArtefactMetadataCriterion(Criterion):
    ...
    type: SearchCriterion = dataclasses.field(default=SearchCriterion.ARTEFACT_METADATA, init=False)
    ...
    def from_dict(cls, entry: dict) -> typing.Self:
        ...
        try:
            ...
            raise ValueError(f'unsupported {op_raw=} for {attr=}')

        mode = cls._parse_mode(entry)

        if op is SearchQueryOperator.EQ:
            if not (value := entry.get('value')):
                raise ValueError(f'missing value for {attr=}')
            return cls(
                attr=str(attr),
                op=op,
                value=str(value),
        ...
        if op is SearchQueryOperator.IN:
            raw_values = entry.get('values')
            if not isinstance(raw_values, list) or not raw_values:
                raise ValueError(f'op=in requires non-empty values for {attr}')
            return cls(
                attr=str(attr),
                op=op,
        ...
        if op is SearchQueryOperator.RANGE:
        ...
        if op is SearchQueryOperator.CMP:
            if not (cmp_op := entry.get('cmp')):
                raise ValueError(f'op=cmp requires cmp for {attr}')

            if not (value := entry.get('value')):
                raise ValueError(f'op=cmp requires value for {attr}')
        ...
        raise ValueError(f'unsupported op {op_raw} for {attr=}')
...
class FulltextCriterion(Criterion):
    ...
    type: SearchCriterion = dataclasses.field(default=SearchCriterion.FULLTEXT, init=False)

    @classmethod
    def from_dict(cls, entry: dict) -> typing.Self:
        value = entry.get('value')
        if not value:
            raise ValueError('fulltext entry missing value')

        fields = entry.get('fields', None)
        if fields is not None:
            if not isinstance(fields, list) or not all(isinstance(f, str) for f in fields):
                raise ValueError('fulltext fields must be a list of strings')
...
def parse_criterion(entry: dict) -> CriterionType:
    if not isinstance(entry, dict):
        raise ValueError('each criteria entry must be an dict')

    type_raw = entry.get('type')
    if not type_raw:
        raise ValueError('criteria entry missing type')

    try:
        criteria_type = SearchCriterion(str(type_raw))
    except Exception:
        raise ValueError(f'unsupported criteria {type_raw=}')

    factories: dict[SearchCriterion, collections.abc.Callable[[dict], CriterionType]] = {
        SearchCriterion.OCM: OcmCriterion.from_dict,
        SearchCriterion.FULLTEXT: FulltextCriterion.from_dict,
        SearchCriterion.ARTEFACT_METADATA: ArtefactMetadataCriterion.from_dict,
    }
...
class SortSpec:
    ...
    def from_dict(cls, entry: dict) -> typing.Self:
        if not isinstance(entry, dict):
            raise ValueError('each sort entry must be an dict')

        field = entry.get('field')
        if not field:
            raise ValueError('sort entry missing field')

        order_raw = entry.get('order') or SortOrder.ASC.value
        try:
            order = SortOrder(str(order_raw).lower())
        except Exception:
            raise ValueError(f'invalid sort {order_raw=}')
...
class SearchRequest:
    ...
    def from_dict(body: dict) -> typing.Self:
        if not isinstance(body, dict):
            raise ValueError('request body must be an dict')
        ...
        if cursor is not None and not isinstance(cursor, dict):
            raise ValueError('cursor must be an dict')
