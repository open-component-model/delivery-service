...
class RoleBindingOrigin(enum.StrEnum):
    '''
...
class RoleBinding:
    name: str
    origin: RoleBindingOrigin

    def __hash__(self) -> int:
        return hash(f'{self.name}|{self.origin}')
...
class UserIdentifier:
    ...
    def normalised_digest(self) -> str:
        return hashlib.sha1(self.normalised.encode('utf-8'), usedforsecurity=False).hexdigest()
