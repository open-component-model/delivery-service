...
def add_app_context_vars(
    ...
    app[consts.ADDRESSBOOK_ENTRIES] = addressbook_entries
    app[consts.ADDRESSBOOK_GITHUB_MAPPINGS] = addressbook_github_mappings
    app[consts.ADDRESSBOOK_SOURCE] = addressbook_source
    app[consts.ARTEFACT_METADATA_CFG] = artefact_metadata_cfg_by_type
    app[consts.BASE_URL] = base_url
    app[consts.CFG_FACTORY] = cfg_factory
    app[consts.COMPONENT_DESCRIPTOR_LOOKUP] = component_descriptor_lookup
    app[consts.COMPONENT_WITH_TESTS_CALLBACK] = component_with_tests_callback
    app[consts.CVE_RESCORING_RULE_SET_LOOKUP] = cve_rescoring_rule_set_lookup
    app[consts.DEFAULT_RULE_SET_CALLBACK] = default_rule_set_callback
    app[consts.DELIVERY_CFG] = parsed_arguments.delivery_cfg
    app[consts.EOL_CLIENT] = eol.EolClient()
    app[consts.GITHUB_API_LOOKUP] = github_api_lookup
    app[consts.GITHUB_REPO_LOOKUP] = github_repo_lookup
    app[consts.INVALID_SEMVER_OK] = parsed_arguments.invalid_semver_ok
    app[consts.ISSUE_REPO_CALLBACK] = issue_repo_callback
    app[consts.KUBERNETES_API_CALLBACK] = kubernetes_api_callback
    app[consts.NAMESPACE_CALLBACK] = namespace_callback
    app[consts.OCI_CLIENT] = oci_client
    app[consts.SERVICE_EXTENSIONS_CALLBACK] = service_extensions_callback
    app[consts.SPECIAL_COMPONENT_CALLBACK] = special_component_callback
    app[consts.SPRINT_DATE_DISPLAY_NAME_CALLBACK] = sprint_date_display_name_callback
    app[consts.SPRINTS] = sprints
    app[consts.SPRINTS_METADATA] = sprints_metadata
    app[consts.UPR_REGEX_CALLBACK] = upr_regex_callback
    app[consts.VERSION_FILTER_CALLBACK] = version_filter_callback
    app[consts.VERSION_LOOKUP] = version_lookup
