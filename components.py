...
class Component(aiohttp.web.View):
    async def get(self):
        ...
        component_descriptor = await _component_descriptor(
            ...
            version_filter=self.request.app[consts.VERSION_FILTER_CALLBACK](),
            invalid_semver_ok=self.request.app[consts.INVALID_SEMVER_OK],
        )
...
class ComponentDependencies(aiohttp.web.View):
    async def get(self):
        ...
        version_filter = util.param(
            params=params,
            name='version_filter',
            default=self.request.app[consts.VERSION_FILTER_CALLBACK](),
        )
        util.get_enum_value_or_raise(version_filter, features.VersionFilter)
...
class ComponentResponsibles(aiohttp.web.View):
    async def get(self):
        ...
        **response:**
        ...
        user_identities = [
            yp.inject(
                addressbook_source=self.request.app[consts.ADDRESSBOOK_SOURCE],
                addressbook_entries=self.request.app[consts.ADDRESSBOOK_ENTRIES],
                addressbook_github_mappings=self.request.app[consts.ADDRESSBOOK_GITHUB_MAPPINGS],
                user_id=user_id,
            ).identifiers
...
class UpgradePRs(aiohttp.web.View):
    ...
    async def get(self):
        ...
        gh_api = self.request.app[consts.GITHUB_API_LOOKUP](
            repo_url,
            absent_ok=True,
        )
        if not gh_api:
        ...
        upgrade_prs: collections.abc.Iterable[github.util.UpgradePullRequest] = pr_helper.enumerate_upgrade_pull_requests( # noqa:E501
            state=pr_state,
            pattern=self.request.app[consts.UPR_REGEX_CALLBACK](),
        )
...
class Issues(aiohttp.web.View):
    ...
    async def get(self):
        ...
        issue_repo = self.request.app[consts.ISSUE_REPO_CALLBACK](component_name)
        ...
        ls_repo = self.request.app[consts.GITHUB_REPO_LOOKUP](issue_repo)
...
class ComplianceSummary(aiohttp.web.View):
    ...
    async def get(self):
        ...
        **response:**
        ...
        '''
        params = self.request.rel_url.query
        artefact_metadata_cfg_by_type = self.request.app[consts.ARTEFACT_METADATA_CFG]
        component_descriptor_lookup = self.request.app[consts.COMPONENT_DESCRIPTOR_LOOKUP]
        eol_client = self.request.app[consts.EOL_CLIENT]
        invalid_semver_ok = self.request.app[consts.INVALID_SEMVER_OK]
        version_filter_callback = self.request.app[consts.VERSION_FILTER_CALLBACK]
        version_lookup = self.request.app[consts.VERSION_LOOKUP]
        ...
        db_session: sqlasync.session.AsyncSession = self.request[consts.DB_SESSION]
