...
def is_os_id_empty(
    ...
    return not any(dataclasses.asdict(os_id).values())
...
def is_one_patch_behind(
    ...
    greatest_version = version.parse_to_semver(greatest_version)
...
def determine_os_status(
    os_id: um.OperatingSystemId,
    release_infos: list[osinfo.model.OsReleaseInfo],
) -> tuple[dso.model.OsStatus, str | None, datetime.datetime | None]:
    if is_os_id_empty(os_id):
    ...
    branch_reached_eol = osidutil.branch_reached_eol(
        os_id=os_id,
        os_infos=release_infos,
    )
    if branch_reached_eol:
    ...
    return dso.model.OsStatus.GREATEST_BRANCH_VERSION, greatest_version, eol_date


def severity_for_os_status(
    os_status: dso.model.OsStatus,
...
def create_artefact_metadata(
    ...
    if not release_infos:
        os_status = dso.model.OsStatus.NO_RELEASE_INFO
