...
class OAuthCfgs(aiohttp.web.View):
    async def get(self):
        def oauth_cfg_to_dict(oauth_cfg: model.delivery.OAuth):
            cfg_factory = self.request.app[consts.CFG_FACTORY]
            ...
            redirect_uri = ci.util.urljoin(
                self.request.app[consts.BASE_URL],
                'auth',
...
class OAuthLogin(aiohttp.web.View):
    async def get(self):
        ...
        delivery_cfg = cfg_factory.delivery(self.request.app[consts.DELIVERY_CFG])
        ...
        token = {
            'version': 'v1',
            'sub': user['login'],
            'iss': self.request.app[consts.BASE_URL],
...
class OpenIDCfg(aiohttp.web.View):
    ...
    '''
    async def get(self):
        base_url = self.request.app[consts.BASE_URL]
...
def auth_middleware(
    ...
    async def middleware(
        ...
        issuer = request.app[consts.BASE_URL]
        ...
        request[consts.GITHUB_USER] = GithubUser(
            username=subject,
        ...
        if github_oAuth:
            request[consts.USER_PERMISSIONS] = get_permissions_for_github_oAuth(github_oAuth)
        else:
            request[consts.USER_PERMISSIONS] = get_user_permissions(subject)
