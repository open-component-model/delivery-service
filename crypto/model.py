...
def filter_crypto_assets(
    components: collections.abc.Iterable[dict] | None,
...
def determine_certificate_kind(
...
def deserialise_certificate(
    ...
    kind = determine_certificate_kind(
...
def deserialise_crypto_asset(
    ...
    if type is ComponentTypes.LIBRARY:
        ...
        raise ValueError(
            ...
            f'supported values: {[asset_type.value for asset_type in dso.model.AssetTypes]}'
        )

    if asset_type is dso.model.AssetTypes.ALGORITHM:
        properties = deserialise_algorithm(
            name=name,
            algorithm_properties=crypto_properties['algorithmProperties'],
        )

    elif asset_type is dso.model.AssetTypes.CERTIFICATE:
        properties = deserialise_certificate(
            certificate_properties=crypto_properties['certificateProperties'],
            crypto_assets_raw=crypto_assets_raw,
        )

    elif asset_type is dso.model.AssetTypes.LIBRARY:
        properties = dso.model.LibraryProperties(
            name=name,
            version=version,
        )

    elif asset_type is dso.model.AssetTypes.PROTOCOL:
    ...
    elif asset_type is dso.model.AssetTypes.RELATED_CRYPTO_MATERIAL:
