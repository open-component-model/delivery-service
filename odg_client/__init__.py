...
class DeliveryServiceRoutes:
    ...
    def auth(self):
        return odg_client.util.urljoin(
            self._base_url,
            'auth',
        )
...
class DeliveryServiceClient:
    def __init__(
        ...
        self.auth_credentials: om.GitHubAuthCredentials = None  # filled lazily as needed
    ...
    def _authenticate(self):
        if self._bearer_token and not odg_client.jwt.is_jwt_token_expired(
        ...
        if (
            ...
            and odg_client.jwt.is_jwt_token_expired(
        ...
        if not self.auth_credentials:
            ...
            self.auth_credentials = om.GitHubAuthCredentials(
                api_url=api_url,
        ...
        self._bearer_token = res.cookies.get(odg_client.jwt.JWT_KEY)
    ...
    def update_metadata(
        ...
        data, headers = odg_client.util.encode_request(
            json={
                'entries': [
                    dataclasses.asdict(
                        artefact_metadata,
                        dict_factory=odg_client.util.dict_to_json_factory,
                    )
    ...
    ) -> tuple[list[dict] | None, list[om.Status] | None]:
        """
...
def _normalise_github_hostname(github_url: str):
    ...
    github_hostname = odg_client.util.urlparse(github_url).hostname
