...
class StandardRef:
    ...
    def retrieve_standard(self) -> crypto.config.Standard:
        crypto_cfg_raw = self.ref.retrieve()
        ...
        for standard_raw in standards_raw:
            if (
                ...
                return dacite.from_dict(
                    data_class=crypto.config.Standard,
                    data=standard_raw,
...
class CryptoMapping(Mapping):
    ...
    :param str aws_secret_name:
        Name of the AWS secret element to use to retrieve artefacts from S3.
    '''
    standards: list[StandardRef | crypto.config.Standard]
    ...
    def __post_init__(self):
        self.standards = [
            standard.retrieve_standard() if isinstance(standard, StandardRef) else standard
        ...
        for library in self.libraries:
            ...
            libraries.extend(library.ref.retrieve().get('libraries', []))
